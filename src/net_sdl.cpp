...
static void NET_SDL_InitAddrTable(void)
{
    ...
    addr_table = Z_Malloc<decltype(*addr_table)>(sizeof(addrpair_t *) * addr_table_size,
                          PU_STATIC, 0);
...
static net_addr_t *NET_SDL_FindAddress(IPaddress *addr)
{
    ...
    if (empty_entry < 0)
    {
        ...
        new_addr_table = Z_Malloc<decltype(*new_addr_table)>(sizeof(addrpair_t *) * new_addr_table_size,
                                  PU_STATIC, 0);
    ...
    new_entry = Z_Malloc<decltype(*new_entry)>(sizeof(addrpair_t), PU_STATIC, 0);

...
wad_file_t *W_AddFile (const char *filename)
{
	...
	fileinfo = Z_Malloc<decltype(*fileinfo)>(sizeof(filelump_t), PU_STATIC, 0);
	...
	fileinfo = Z_Malloc<decltype(*fileinfo)>(length, PU_STATIC, 0);
...
void W_GenerateHashTable(void)
{
    ...
    if (numlumps > 0)
    {
        lumphash = Z_Malloc<decltype(*lumphash)>(sizeof(lumpindex_t) * numlumps, PU_STATIC, NULL);

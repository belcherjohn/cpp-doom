...
void SN_InitSequenceScript(void)
{
    ...
    while (SC_GetString())
    {
        if (*sc_String == ':')
        {
            ...
            tempDataStart = Z_Malloc<decltype(*int )>(SS_TEMPBUFFER_SIZE,
                                             PU_STATIC, NULL);
        ...
        else if (SC_Compare(SS_STRING_END))
        {
            ...
            *tempDataPtr++ = SS_CMD_END;
            ...
            SequenceData[i] = Z_Malloc<decltype(*int )>(dataSize, PU_STATIC, NULL);
...
void SN_StartSequence(mobj_t * mobj, int sequence)
{
    ...
    node = Z_Malloc<decltype(*seqnode_t )>(sizeof(seqnode_t), PU_STATIC, NULL);

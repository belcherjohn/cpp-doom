...
static void InitHashTable(void)
{
    ...
    hash_table = Z_Malloc<decltype(*hash_table)>(sizeof(deh_substitution_t *) * hash_table_length,
                          PU_STATIC, NULL);
...
void DEH_AddStringReplacement(const char *from_text, const char *to_text)
{
    ...
    if (sub != NULL)
    {
        ...
        sub->to_text = Z_Malloc<decltype(*to_text)>(len, PU_STATIC, NULL);
        memcpy(sub->to_text, to_text, len);
    }
    else
    {
        // We need to allocate a new substitution.
        sub = Z_Malloc<decltype(*sub)>(sizeof(*sub), PU_STATIC, 0);

        // We need to create our own duplicates of the provided strings.
        len = strlen(from_text) + 1;
        sub->from_text = Z_Malloc<decltype(*from_text)>(len, PU_STATIC, NULL);

  ...
  int		height )
{
    ...
    dest = Z_Malloc<decltype(*short)>(width*height*2, PU_STATIC, 0);
  ...
  int	ticks )
{
    ...
    y = Z_Malloc<decltype(*int )>(width*sizeof(int), PU_STATIC, 0);
    y[0] = -(M_Random()%16);
    for (i=1;i<width;i++)
    {
	r = (M_Random()%3) - 1;
...
( int	x,
  ...
  int	height )
{
    wipe_scr_start = Z_Malloc<decltype(*wipe_scr_start)>(SCREENWIDTH * SCREENHEIGHT, PU_STATIC, NULL);
...
( int	x,
  ...
  int	height )
{
    wipe_scr_end = Z_Malloc<decltype(*wipe_scr_end)>(SCREENWIDTH * SCREENHEIGHT, PU_STATIC, NULL);
  ...
  int	ticks )
{
	...
	wipe_scr = Z_Malloc<decltype(*byte )>(width*height, PU_STATIC, 0); // DEBUG
